        ...
        return Div.create().className("doc-page")
            ...
            .child(Section.create().className("doc-section")
                ...
                .child(p("teavm-lambda supports multiple deployment targets. The same application "
                    + "code works with all of them \u2014 only the Maven dependencies and profile "
                    + "change. The project generated by the initializer on the Installation page "
                    + "is configured for the JVM standalone server. Below we show how to run it "
                    + "locally with each target."))

                // JVM Standalone
                .child(h3("JVM Standalone Server"))
                .child(p("The simplest way to run locally. Build an uber JAR with the built-in "
                    + "HTTP server \u2014 no Docker or cloud tooling needed. This works out of "
                    + "the box with the starter project."))
                .child(CodeBlock.create(
                    """
                    mvn clean package
                ...
                .child(p("To run as an AWS Lambda function locally, you need to add the "
                    + "Lambda adapter dependency to your pom.xml:"))
                .child(CodeBlock.create(
                    """
                    <dependency>
                    ...
                    "markup"))
                .child(p("Then create a template.yaml in your project root:"))
                .child(CodeBlock.create(
                    """
                ...
                .child(Callout.deepDive("TeaVM / Node.js Lambda",
                    p("The example above uses the JVM Lambda runtime (java21). For ~100ms "
                        + "cold starts, teavm-lambda can also compile your Java code to JavaScript "
                        + "and run on Node.js 22. This requires a different build configuration "
                        + "with the TeaVM compiler plugin. See the "),
                    a("Deployment").href("#/learn/deployment").build(),
                    p(" page for the full TeaVM Lambda setup.")))
